...
impl<T, K> From<T> for Notes
    where T: IntoIterator<Item = K>, K: Into<u8> {
...
impl Key {
    ...
    pub fn scale(&self) -> Vec<u8> {
        self.scale.relative_pitches().iter().map(|p| ((self.tonic % 12) + p) % 12).collect()
    }
...
impl Chord {
    ...
    pub fn values() -> Vec<Chord> {
        vec![Self::I, Self::II, Self::III, Self::IV, Self::V, Self::VI, Self::VII]
    }
...
impl From<&Chord> for String {
    fn from(value: &Chord) -> Self { Self::from(*value) }
}
...
impl Scale {
    ...
    pub fn values() -> Vec<Scale> {
        vec![Self::Major, Self::Minor, Self::NaturalMinor, Self::HarmonicMinor]
    }
...
impl From<&Scale> for String {
    fn from(value: &Scale) -> Self { Self::from(*value) }
}
