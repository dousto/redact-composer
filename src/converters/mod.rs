...
use midly::{Smf, Header, Format::Parallel, Timing::Metrical, TrackEvent, TrackEventKind, MidiMessage, MetaMessage};

use crate::composer::{Node, RenderSegment, SegmentType};

pub struct MidiConverter<T> { marker: PhantomData<T> }
...
impl<T> MidiConverter<T> {
    pub fn convert(tree: &Vec<Node<RenderSegment<T>>>) -> Smf {
        ...
        let meta_track = vec![
            TrackEvent { delta: 0.into(), kind: TrackEventKind::Meta(MetaMessage::Tempo(500_000.into())) }
        ];

        let mut tracks: Vec<Vec<TrackEvent>> = track_subtrees.iter().enumerate()
        .map(|(idx, tree)| {
            let u8idx: u8 = idx.try_into().unwrap();
            let mut track = Self::convert_subtree(tree, u8idx + 1);
            track.insert(0, TrackEvent { delta: 0.into(), kind: TrackEventKind::Midi { channel: u8idx.into(), message: MidiMessage::ProgramChange { program: 0.into() } } });
            track
        })
        .collect();
        ...
        Smf {
            header: Header { format: Parallel, timing: Metrical(480.into()) },
            tracks: tracks
        }
    }

    fn convert_subtree(tree: &Vec<Node<RenderSegment<T>>>, channel: u8) -> Vec<TrackEvent> {
        let mut abs_time_events: Vec<(u32, TrackEvent)> = tree.iter()
            ...
            SegmentType::PlayNote { note, velocity } => Some([
                    ...
                    (
                        ...
                        TrackEvent {
                            ...
                            kind: TrackEventKind::Midi {
                                channel: channel.into(),
                                message: MidiMessage::NoteOff { key: note.into(), vel: velocity.into() }
                            },
                        }
                    ),
                ]),
